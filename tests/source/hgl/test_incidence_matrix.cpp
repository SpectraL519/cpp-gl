...
struct test_incidence_matrix {
    ...
    typename IncidenceMatrix::hypergraph_storage_type& storage(IncidenceMatrix& sut
    ) const noexcept {
...
TEST_CASE_FIXTURE(
    ...
    CHECK(std::ranges::all_of(storage(sut), [](const auto& row) {
...
TEST_CASE_FIXTURE(
    ...
    REQUIRE(std::ranges::all_of(storage(sut), [](const auto& row) { return row.size() == 0uz; }));
...
TEST_CASE_FIXTURE(
    ...
    CHECK_EQ(storage(sut)[vertex_id].size(), n_hyperedges - 1uz);
...
TEST_CASE_FIXTURE(
    ...
    CHECK(storage(sut)[constants::id1][constants::id1]);
...
TEST_CASE_FIXTURE(
    ...
    REQUIRE(storage(sut)[constants::id1][constants::id1] == true);
    ...
    CHECK(storage(sut)[constants::id1][constants::id1] == false);
}
...
TEST_CASE_FIXTURE(
    ...
    sut.remove_vertex(rem_vid);
    CHECK_EQ(storage(sut)[hyperedge_id].size(), n_vertices - 1uz);
    CHECK_EQ(sut.hyperedge_size(hyperedge_id), expected_hyperedge_size);
    CHECK(std::ranges::equal(sut.incident_vertices(hyperedge_id), expected_vertices));
}
