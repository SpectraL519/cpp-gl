...
struct test_undirected_incidence_matrix : public test_incidence_matrix {
    using sut_type = hgl::impl::undirected_incidence_matrix;
};

TEST_CASE_FIXTURE(test_undirected_incidence_matrix, "should initialize empty matrix by default") {
    sut_type sut{};
    CHECK(matrix(sut).empty());
}

TEST_CASE_FIXTURE(
    test_undirected_incidence_matrix,
...
TEST_CASE_FIXTURE(test_undirected_incidence_matrix, "add_vertices should properly extend matrix") {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
...
TEST_CASE_FIXTURE(
    test_undirected_incidence_matrix, "incident_vertices should return an empty view by default"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
...
TEST_CASE_FIXTURE(test_undirected_incidence_matrix, "hyperedge_size should return 0 by default") {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
    CHECK(std::ranges::all_of(constants::hyperedge_ids_view, [&sut](const auto hyperedge_id) {
...
TEST_CASE_FIXTURE(test_undirected_incidence_matrix, "bind should set the corresponding bit") {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
...
TEST_CASE_FIXTURE(test_undirected_incidence_matrix, "unbind should clear the corresponding bit") {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
...
TEST_CASE_FIXTURE(
    test_undirected_incidence_matrix, "are_bound should return true only when the bit is set"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
