...
TEST_SUITE_BEGIN("test_flat_incidence_list");

struct test_flat_incidence_list {
    template <typename FlatIncidenceList>
    using storage_type = typename FlatIncidenceList::storage_type;

    template <typename FlatIncidenceList>
    requires std::same_as<typename FlatIncidenceList::directional_tag, hgl::undirected_t>
    storage_type<FlatIncidenceList>& storage(FlatIncidenceList& sut) const noexcept {
        return sut._storage;
    }

    template <typename FlatIncidenceList>
    requires std::same_as<typename FlatIncidenceList::directional_tag, hgl::bf_directed_t>
    storage_type<FlatIncidenceList>& tail_storage(FlatIncidenceList& sut) const noexcept {
        return sut._tail_storage;
    }

    template <typename FlatIncidenceList>
    requires std::same_as<typename FlatIncidenceList::directional_tag, hgl::bf_directed_t>
    storage_type<FlatIncidenceList>& head_storage(FlatIncidenceList& sut) const noexcept {
        return sut._head_storage;
    }
};

struct test_undirected_vertex_major_flat_incidence_list : public test_flat_incidence_list {
    using impl_tag = hgl::repr::flat_list_t<hgl::repr::vertex_major_t>;
    using sut_type = hgl::impl::flat_incidence_list<hgl::undirected_t, impl_tag>;
};

TEST_CASE_FIXTURE(
    test_undirected_vertex_major_flat_incidence_list, "should initialize empty storage by default"
) {
    sut_type sut{};
...
TEST_CASE_FIXTURE(
    test_undirected_vertex_major_flat_incidence_list, "degree should return 0 by default"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
    CHECK(std::ranges::all_of(constants::vertex_ids_view, [&sut](const auto vertex_id) {
...
TEST_CASE_FIXTURE(
    test_undirected_vertex_major_flat_incidence_list, "add_hyperedges should do nothing"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
...
struct test_undirected_hyperedge_major_flat_incidence_list : public test_flat_incidence_list {
    using impl_tag = hgl::repr::flat_list_t<hgl::repr::hyperedge_major_t>;
    using sut_type = hgl::impl::flat_incidence_list<hgl::undirected_t, impl_tag>;
};

TEST_CASE_FIXTURE(
    test_undirected_hyperedge_major_flat_incidence_list,
    "should initialize empty storage by default"
) {
    sut_type sut{};
    CHECK(storage(sut).empty());
}
...
TEST_CASE_FIXTURE(
    test_undirected_hyperedge_major_flat_incidence_list, "hyperedge_size should return 0 by default"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
    CHECK(std::ranges::all_of(constants::hyperedge_ids_view, [&sut](const auto hyperedge_id) {
...
TEST_CASE_FIXTURE(
    test_undirected_hyperedge_major_flat_incidence_list, "add_vertices should do nothing"
) {
    sut_type sut{};
    sut.add_vertices(constants::n_vertices);
    CHECK(storage(sut).empty());
...
TEST_CASE_FIXTURE(
    test_bf_directed_vertex_major_flat_incidence_list, "should initialize an empty list by default"
) {
    sut_type sut{};
...
TEST_CASE_FIXTURE(
    test_bf_directed_vertex_major_flat_incidence_list,
    "add_vertices should properly extend the list"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
...
TEST_CASE_FIXTURE(
    test_bf_directed_vertex_major_flat_incidence_list, "add_hyperedges should do nothing"
) {
...
TEST_CASE_FIXTURE(
    test_bf_directed_vertex_major_flat_incidence_list,
    "bind_tail should insert the vertex id to the tail list of an appropriate hyperedge entry"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
    REQUIRE(std::ranges::empty(sut.incident_vertices(constants::id1)));
...
TEST_CASE_FIXTURE(
    test_bf_directed_vertex_major_flat_incidence_list,
    "bind_head should insert the vertex id to the head list of an appropriate hyperedge entry"
) {
    sut_type sut{constants::n_vertices, constants::n_hyperedges};
    REQUIRE(std::ranges::empty(sut.incident_vertices(constants::id1)));
...
TEST_CASE_FIXTURE(
    ...
    SUBCASE("tail bound") {
        sut.bind_tail(constants::id1, constants::id1);
        expected_storage = tail_storage(sut)[constants::id1] | std::ranges::to<std::vector>();
    }
    SUBCASE("head bound") {
        sut.bind_head(constants::id1, constants::id1);
        expected_storage = head_storage(sut)[constants::id1] | std::ranges::to<std::vector>();
    }
    CAPTURE(sut);
...
TEST_CASE_FIXTURE(
    ...
    "should initialize an empty list by default"
) {
    sut_type sut{};
    CHECK(tail_storage(sut).empty());
    CHECK(head_storage(sut).empty());
...
TEST_CASE_FIXTURE(
    test_bf_directed_hyperedge_major_flat_incidence_list, "add_vertices should do nothing"
) {
...
TEST_SUITE_END(); // test_flat_incidence_list
