...
public:
    ...
    ///          Time complexity is amortized $O(K)$ where K is the size of the range.
    template <std::ranges::input_range R>
    ...
    ///          Time complexity is $O(1)$ amortized for the container overhead, plus $O(S)$ to truncate
    ///          the underlying `_data` vector where S is the size of the removed segment.
    void pop_back() {
        if (this->empty())
            return;
    ...
    ///          **Time complexity is $O(N + K)$** where N is the total number of elements from `pos` onward
    ///          and K is the size of the inserted range, due to element shifting and offset updates.
    ///          Insertion at the end is amortized $O(K)$.
    template <std::ranges::input_range R>
    requires std::convertible_to<std::ranges::range_reference_t<R>, value_type>
    ...
    ///          and M is the number of elements in ilist. Insertion at the end is amortized $O(M)$.
    ...
    ///          **Time complexity is $O(N)$** where N is the total number of elements from `pos` onward,
    ///          due to element removal and offset updates. Erasing the last segment is $O(S)$ where
    ///          S is the size of the erased segment.
    ...
    /// @warning Invalidates all iterators, pointers, and references to the removed element.
    ///          Time complexity is $O(N - seg)$ where N is the size of the container *after* index seg,
    ///          due to offset updates.
    ...
    /// @post The segment size increases by 1; `data_size()` increases by 1; offsets for subsequent segments updated
    /// @exception std::bad_alloc If memory allocation fails
    /// @warning Invalidates all iterators, pointers, and references to elements at or after the insertion point.
    ///          **Time complexity is $O(N - seg + pos)$** where N is the total number of elements after
    ///          the insertion point within the flattened vector, plus offset updates for all subsequent segments.
    ///          Appending to the end of the segment is amortized $O(1)$.
    ...
    /// @exception Any exception thrown by the `T` constructor, or std::bad_alloc
    /// @warning Invalidates all iterators, pointers, and references to elements at or after the insertion point.
    ///          **Time complexity is $O(N - seg + pos)$** due to element shifting and offset updates.
    template <class... Args>
    ...
    /// @post The segment size decreases by 1; `data_size()` decreases by 1; offsets for subsequent segments updated
    /// @warning Invalidates all iterators, pointers, and references to elements at or after the removed position.
    ///          **Time complexity is $O(N - seg - pos)$** where N is the total number of elements after
    ///          the erased position, plus offset updates for all subsequent segments.
