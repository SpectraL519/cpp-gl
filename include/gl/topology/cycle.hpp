...
[[nodiscard]] GraphType cycle(size_type n_vertices) {
    ...
    for (auto source_id = constants::initial_id_v<id_type>; source_id < n_vertices; ++source_id)
...
[[nodiscard]] GraphType bidirectional_cycle(size_type n_vertices) {
    if constexpr (traits::c_directed_graph<GraphType>) {
        ...
        for (auto source_id = constants::initial_id_v<id_type>; source_id < n_vertices;
             ++source_id) {
