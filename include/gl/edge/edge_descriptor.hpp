...
template <detail::u_integral VertexKey, detail::satisfies_or_void<detail::is_equality_comparable> Data>
...
template <typename Descriptor>
concept data_edge_descriptor_c =
    edge_descriptor_c<Descriptor> && data_descriptor_t<Descriptor>;

template <typename Descriptor, typename VertexKey>
concept key_type_matching_edge_descriptor_c =
    edge_descriptor_c<Descriptor> &&
    std::is_same_v<typename Descriptor::vertex_key_type, VertexKey>;


template <detail::u_integral VertexKey = std::size_t, detail::satisfies_or_void<detail::is_equality_comparable> Data = void>
class edge_descriptor {
public:
    ...
    explicit edge_descriptor(
        ...
        : source(source), destination(destination), _data(data) {}

    edge_descriptor(const type& other)
        : source(other.source), destination(other.destination), _data(other._data) {}
    ...
    inline void set_data(const data_type& data) { this->_data = data; }
