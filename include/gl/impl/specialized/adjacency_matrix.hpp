...
inline void check_edge_override(const auto& id_matrix, id_type source_id, id_type target_id) {
    if (const auto edge_id = id_matrix[source_id][target_id];
        edge_id != constants::invalid_id<id_type>)
...
struct directed_adjacency_matrix {
    ...
    [[nodiscard]] gl_attr_force_inline static size_type in_degree(
        ...
        return std::ranges::count_if(self._matrix, [vertex_id](const auto& row) {
            return row[vertex_id] != constants::invalid_id<id_type>;
        });
    }
    ...
    [[nodiscard]] gl_attr_force_inline static size_type out_degree(
        ...
        return self._matrix[vertex_id].size()
             - std::ranges::count(self._matrix[vertex_id], constants::invalid_id<id_type>);
    }
    ...
    [[nodiscard]] gl_attr_force_inline static size_type degree(
        ...
        size_type deg = 0uz;
        for (auto v_id = 0uz; v_id < self._matrix.size(); ++v_id)
            deg += static_cast<
                       size_type>(self._matrix[vertex_id][v_id] != constants::invalid_id<id_type>)
                 + static_cast<
                       size_type>(self._matrix[v_id][vertex_id] != constants::invalid_id<id_type>);
        ...
        for (const auto& row : self._matrix)
            for (auto [target_id, edge_id] : std::views::enumerate(row))
                in_degree_map[target_id] +=
                    static_cast<size_type>(edge_id != constants::invalid_id<id_type>);

        return in_degree_map;
    }

    [[nodiscard]] static std::vector<size_type> out_degree_map(const impl_type& self) {
        return std::views::iota(constants::initial_id<id_type>, self._matrix.size())
    ...
    [[nodiscard]] static std::vector<size_type> degree_map(const impl_type& self) {
        ...
        for (auto source_id = 0uz; source_id < self._matrix.size(); ++source_id) {
            for (auto target_id = 0uz; target_id < self._matrix.size(); ++target_id) {
                if (self._matrix[source_id][target_id] != constants::invalid_id<id_type>) {
    ...
    static std::vector<id_type> remove_vertex(impl_type& self, id_type vertex_id) {
        auto removed_edges =
            self._matrix[vertex_id] | std::views::filter([](auto edge_id) {
        ...
        for (auto& row : self._matrix) {
            if (const auto edge_id = row[vertex_id]; edge_id != constants::invalid_id<id_type>)
    ...
    static inline void remove_edge(impl_type& self, const edge_type& edge) {
        detail::strict_get(self._matrix, edge) = constants::invalid_id<id_type>;
    }
};
...
struct undirected_adjacency_matrix {
    ...
    [[nodiscard]] gl_attr_force_inline static size_type degree(
        ...
        return self._matrix.size()
             - std::ranges::count(self._matrix[vertex_id], constants::invalid_id<id_type>)
             + static_cast<
                   size_type>(self._matrix[vertex_id][vertex_id] != constants::invalid_id<id_type>);
    }
    ...
    [[nodiscard]] static std::vector<size_type> degree_map(const impl_type& self) {
        ...
        for (auto source_id = 0uz; source_id < self._matrix.size(); ++source_id) {
            for (auto target_id = 0uz; target_id <= source_id; ++target_id) {
                if (self._matrix[source_id][target_id] != constants::invalid_id<id_type>) {
                    ++degree_map[source_id];
                    ++degree_map[target_id];
                }
    ...
    static void remove_edge(impl_type& self, const edge_type& edge) {
        if (edge.is_loop()) {
            ...
            self._matrix[edge.target()][edge.source()] = constants::invalid_id<id_type>;
        }
    }
};
