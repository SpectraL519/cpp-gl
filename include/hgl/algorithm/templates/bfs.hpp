...
/// | InitQueueRangeType | A forward range of `search_node<hypergraph_val_t<H>>` used to prime the BFS queue. | Must be a *forward range* of @ref hgl::algorithm::search_node "search nodes". |
...
/// | EnqueuePredicate | Type of the callable deciding if a target vertex should be enqueued via a specific hyperedge. | Must be one of:<br/>- A `(const search_node<hypergraph_val_t<H>>&) -> decision` callable<br/>- An @ref hgl::algorithm::empty_callback "empty_callback" |
...
bool bfs(
    ...
    std::queue<search_node<hypergraph_val_t<H>>> q;
